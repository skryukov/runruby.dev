...
const mapFilesToFS = (files: { filename: string, content: string }[]): FileSystemTree => {
...
window.addEventListener("load", async () => {
  ...
  const snapshotResponse = await fetch(`${import.meta.env.VITE_ASSETS_URL}/ruby`);
  ...
  const snapshotResponse1 = await fetch(`${import.meta.env.VITE_ASSETS_URL}/stubs`);
...
export const startShell = async (terminal: Terminal) => {
  ...
  const shellProcess = await webcontainerInstance.spawn("jsh", {
    ...
    env: { "PATH": `${webcontainerInstance.workdir}/bin:${webcontainerInstance.path}` }
  });
